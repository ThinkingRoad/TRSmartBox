/*
  ...
  LAST Box UPLOADED: ~/Desktop/TRSmartBox/TRSmartBox.ino Aug 21 2017 20:09:31  IDE 10803
...
#define SmartBox 999
#define RelayPin 5 // SSR vcc++ via logic high
...
int thermoDO = 7, thermoCS = 8, thermoCLK = 9, vccPin = 10, gndPin = 11;
MAX6675 thermocouple(thermoCLK, thermoCS, thermoDO);
#endif


int val, redVal, greenVal, blueVal;
int rgbPin[] = {2, 3, 4};  // RGB LED PINS
int rgbVal[] = {0, 0, 0};     // RGB - HSV PWM 0-255

int controlMode =0; // set this to 0 to turn off the unit
...
void setup() {
  ...
  u8x8.draw2x2String(0, 0, "SmartBox");
#endif
...
void debugControls() {  // enable this in void loop for an emergency override
  potValue = 0; // set this to 10 or less to turn off the unit
...
void loop() {
  ...
  setpointVal =  map(potValue, 0, 1023, 0, 999); // convert 0-1023 potValue to a setpointVal of 109-1075
...
//  if (setpointVal < 109) setpointVal = 109; // chop that data value off at the knees
  if (setpointVal > 999) setpointVal = 999; // chop that data value off at the knees
  ...
  //  //               thinkingroad.org
  //  u8x8.setCursor(2+9.8, 5);
  //  if (procVal > 100) u8x8.setCursor(2+9.8, 5);
  //  else u8x8.setCursor(2+10.8, 5);
  //  u8x8.print(fixFloat);
  //  }
  ...
  */
  ...
  u8x8.draw2x2String(2, 2, "SP");
  
  itoa(setpointVal,varToString, 10);
  ...
  u8x8.draw2x2String(2, 4, "PV");

  itoa(procVal,varToString, 10);

  if (procVal >= 1000) {
      u8x8.draw2x2String(6, 4, varToString);
  } else if (procVal >= 100)  { 
      u8x8.draw2x2String(6, 4, " "); 
      u8x8.draw2x2String(8, 4, varToString);
  } else if (procVal < 10) {  
    u8x8.draw2x2String(10, 4, " "); 
    u8x8.draw2x2String(12, 4, varToString);
  } else {  
    u8x8.draw2x2String(8, 4, " "); 
    u8x8.draw2x2String(10, 4, varToString);
  }
    
  if ((procVal > 109) && (controlMode == 1)) u8x8.draw2x2String(1, 6, "HEATING");
  else if (procVal <= 109) u8x8.draw2x2String(1, 6, "COOL :)");
  else  u8x8.draw2x2String(1, 6, "COOLING");
  
// continue on with display

  /************************************************
    non-text hid stuff to tell humans things
    the colours for the on heat vs off heat aren't very
    compatible colour-wheel-wise but damn this is close.

  ************************************************/

  if (procVal >= setpointVal + 50.00) OrangeRedLed();
  if ((procVal <= setpointVal + 50.00)&&(procVal >= setpointVal + 15.00))  GreenLed();
  if ((procVal <= setpointVal - 15.00) && (procVal >= 109.00)) RedLed();
  if (procVal <= 109.00)  BlueLed();

  analogWrite(rgbPin[0], redVal);
  analogWrite(rgbPin[1], greenVal);
  analogWrite(rgbPin[2], blueVal);
...
*/
  ...
  ////    }
  //
  ////  if (procVal >= setpointVal + 11.00) u8x8.drawStr(2, u8x8Row[5], "resting - please wait");
  ////  if ((procVal <= setpointVal + 11.00)&&(procVal >= setpointVal + 10.00))  u8x8.drawStr(2, u8x8Row[5], "ready to use");
  ////  if ((procVal <= setpointVal - 11.00) && (procVal >= 109.00)) u8x8.drawStr(2, u8x8Row[5], "heating - please wait");
  //  //else u8x8.drawStr(2, u8x8Row[5], "at safe temperatures");
  ...
  ////    u8x8.drawStr(2, u8x8Row[0], "ThinkingRoad.org");
  ////    u8x8.drawStr(2, u8x8Row[1], "SmartBox Pro");
  ////    u8x8.setFont(u8x8_font_courB10_tr);
  //
  //    if (controlMode == 1) { //
  ////      u8x8.drawStr(2,  u8x8Row[2], "Heat:");
...
ISR(TIMER0_COMPA_vect) {
  ...
  oneminute++;
  if (oneminute >= 60000) { // one minute
    
    /* VVV do the thing VVV */ //
