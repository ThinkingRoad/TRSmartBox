...
#define newtimer // causes interrupt flash, timer is borked
...
//  Setpoint =  ((map(potValue)), 0, 1023, 250, 1000); // convert 0-1023 potValue to a setpoint of 250-1000

  if (Setpoint < 300) Setpoint = 300; // chop that data value off at the knees
  if (Setpoint > 950) Setpoint = 950; // chop that data value off at the knees



  potLastValue = potValue;

  potSlopeValue = potLastValue - Setpoint;
...
// MOVED ALL CONTROL OUT OF THIS DISPLAY so it is now flexible!
    ...
    u8g2.drawStr(0, u8g2Row[0],"__________________");
    
    u8g2.drawStr(0, u8g2Row[1],"ThinkingRoad.org");
    //u8g2.print(potValue);

    u8g2.drawStr(0, 16,"__________________");
    
    if (controlMode == 1) { //
      u8g2.drawStr(0,  u8g2Row[2], "Setpoint");
      u8g2.setCursor(80, u8g2Row[2]);
      u8g2.print(Setpoint);
      u8g2.drawStr(0,  u8g2Row[3], "Coil ON");
     u8g2.setCursor(80, u8g2Row[3]);
     u8g2.print(Input);
    } else { //
      u8g2.drawStr(0,  u8g2Row[2], "Setpoint");
      u8g2.drawStr(80, u8g2Row[2], "[OFF]");
      u8g2.drawStr(0,  u8g2Row[3], "Coil OFF");
      u8g2.setCursor(80, u8g2Row[3]);
      u8g2.print(Input);
    }
    ...
    if (abs(Setpoint - Input) < 5) {
      u8g2.drawStr(0, u8g2Row[4],"STABLE");
    } else if (abs(Input - Setpoint > 16)) {
      if (controlMode == 1) u8g2.drawStr(0, u8g2Row[4],"UNSTABLE - WAIT");
      else u8g2.drawStr(0, u8g2Row[4],"COOLING - WAIT");
    } else if ((abs(Setpoint - Input) < 15) || controlMode == 1) {
      u8g2.drawStr(0, u8g2Row[4],"METASTABLE - WAIT");
    } else if (abs(Input - Setpoint < 11)) {
      if (controlMode == 1) u8g2.drawStr(0, u8g2Row[4],"HEATING - WAIT");
      else u8g2.drawStr(0, u8g2Row[4],"COOLING - WAIT");

    } else if (Input > 109) {
      
      if (controlMode == 1) u8g2.drawStr(0, u8g2Row[4],"COIL  [ON]");
      else u8g2.drawStr(0, u8g2Row[4], "COIL [OFF]");
    } else if (Input < 32) {
      u8g2.drawStr(0, u8g2Row[4]," READING - WAIT");
    } else {
      u8g2.drawStr(0, u8g2Row[4],"SAFE TEMPERATURE");
    }

    u8g2.drawStr(0, 49,"__________________");
...
ISR(TIMER0_COMPA_vect){
   ...
   ticks[0]++; // max temperature samples
  Serial.print("ticks[0] ");
  Serial.print(ticks[0]);
  Serial.print(" ");

   if(ticks[0] >= 200){ // clock max chip to 200ms sample time to increase stability
...
#ifdef debug
  ticks[1]++; // WindowSize replacement
  Serial.print("ticks[1] ");
  Serial.print(ticks[1]);
  Serial.print(" ");
 if(ticks[1] >= WindowSize){ // clock to WindowSize
  ticks[1] = 0;
   }  // ticks[1]++ 
#endif
Serial.println(" ");
