...
U8X8_SH1106_128X64_NONAME_HW_I2C u8x8(/* SCL */ 5, /* SDA */ 4);
//SDA  connect A4
//SCL  Connect A5

//int u8g2Row[] = {6, 16, 26, 40, 54, 64};
//int u8g2Col = 70;
...
void loop() {
  ...
  Setpoint =  map(potValue, 0, 1023, 300, 950); // convert 0-1023 potValue to a setpoint of 600-1000
  ...
  */
  ...
  do {
    ...
    u8g2.drawStr(u8g2startPos, u8g2Row[0], "ThinkingRoad.org");
    u8g2.drawStr(u8g2startPos, u8g2Row[1], "Smartbox Pro");
    u8g2.setFont(u8g2_font_courB10_tr);

    if (controlMode == 1) { //
      u8g2.drawStr(u8g2startPos,  u8g2Row[2], "Heat:");
      u8g2.setCursor(u8g2Col, u8g2Row[2]);
      u8g2.print(Setpoint);
    } else { //
      u8g2.drawStr(u8g2startPos,  u8g2Row[2], "Heat:");
      u8g2.drawStr(u8g2Col + 19, u8g2Row[2], "OFF");
    }

    u8g2.drawStr(u8g2startPos,  u8g2Row[3], "Coil:");
    u8g2.setCursor(u8g2Col, u8g2Row[3]);
    u8g2.print(Input);

    u8g2.setFont(u8g2_font_courB08_tr);
    if (Input > 109) u8g2.drawStr(u8g2startPos, u8g2Row[4], "do not touch nail");
    else u8g2.drawStr(u8g2startPos, u8g2Row[4], "all heating elements");

    // continue on with display
//
//    if (controlMode == 1) {
//
//      if (abs(Setpoint - Input)) < 2) u8g2.drawStr(1, u8g2Row[5], "at setpoint");
//
//
//
//
//
//
//      if (abs(Input - Setpoint < 11)) u8g2.drawStr(1, u8g2Row[5], "resting - please wait");
//
//
//
//      if (abs(Input - Setpoint > 11)) u8g2.drawStr(1, u8g2Row[5], "heating - please wait");
//      if (abs(Setpoint - Input)) < 10) u8g2.drawStr(1, u8g2Row[5], "ready to use");
//
//        } else {
//
//      if (abs(Input - Setpoint > 11)) u8g2.drawStr(1, u8g2Row[5], "cooling - please wait");
//      else if ((Input < 451) && (Input > 109)) u8g2.drawStr(1, u8g2Row[5], "cooling - swab ok");
//      else u8g2.drawStr(1, u8g2Row[5], "at safe temperatures");
//
//    }

  if (Input >= Setpoint + 11.00) u8g2.drawStr(u8g2startPos, u8g2Row[5], "resting - please wait");
  if ((Input <= Setpoint + 11.00)&&(Input >= Setpoint + 10.00))  u8g2.drawStr(u8g2startPos, u8g2Row[5], "ready to use");
  if ((Input <= Setpoint - 11.00) && (Input >= 109.00)) u8g2.drawStr(u8g2startPos, u8g2Row[5], "heating - please wait");
  else u8g2.drawStr(u8g2startPos, u8g2Row[5], "at safe temperatures");
