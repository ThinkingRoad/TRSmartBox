...
//int u8x8Col = 70;
int u8x8colOne = 1;
int u8x8colTwo = u8x8colOne + 4;
int u8x8col3 = u8x8colOne + 11;
#endif
...
void loop() {
  ...
  setpointVal =  map(potValue, 0, 1023, 109, 1200); // convert 0-1023 potValue to a setpointVal of 109-1075
  ...
  if (setpointVal > 1200) setpointVal = 1200; // chop that data value off at the knees
  ...
  */
  ...
  u8x8.setCursor(u8x8colOne, 3);
  u8x8.print("SP  ");
  u8x8.setCursor(u8x8colTwo, 3);

  if (setpointVal > 999 ) u8x8.setCursor(u8x8colTwo - 1, 3);
  ...
  u8x8.setCursor(u8x8col3, 5);

  if (controlMode == 1) u8x8.print("ON ");
  else u8x8.print("OFF");

  if (procVal < 0) u8x8.setCursor(u8x8colTwo, 5); //-32 falls in this range so this is logic floor
  ...
  if (procVal >= 1000 ) u8x8.setCursor(u8x8colTwo - 1, 5); //ceiling, max op 1200

  u8x8.print(procVal);

  u8x8.setCursor(u8x8colOne-1, 7);
  if (procVal >= 109) u8x8.print("WARNING:HOT NAIL");
                 else u8x8.print("SAFE TEMPERATURE");
